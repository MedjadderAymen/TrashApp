...
public class adapter_item_sql extends ArrayAdapter<ChallengeSQL> {

    Activity activity;
    int idimg;
    List<ChallengeSQL> items;

    private List<Address> addresses;
    private Geocoder geocoder;


    public adapter_item_sql (Activity activity, int idimg, List<ChallengeSQL> items) {
    ...
    public View getView(final int position, View convertView, @NonNull ViewGroup parent) {
        ...
        Glide.with(activity).load(items.get(position).getOne_path()).into(photo_challenge);

        TextView owner = layout.findViewById(R.id.chal_owner);
        owner.setText(items.get(position).getUser_name());
